...
export default function VocabularyManagerPage() {
  ...
  const loadContributions = useCallback(async () => {
    ...
    try {
      const response = await api<{ items: Contribution[] }>(
        portalPath("center", "vocabulary", "/center/contributions?limit=100"),
  ...
  async function checkWords() {
    ...
    try {
      const response = await api<{ items: CheckResult[] }>(
        portalPath("center", "vocabulary", "/center/words/check"),
        auth.session.access_token,
        json("POST", { words }),
      );
  ...
  async function addSingle() {
    ...
    try {
      const response = await api<CheckResult>(
        portalPath("center", "vocabulary", "/center/words"),
        auth.session.access_token,
        json("POST", {
  ...
  async function addBulk() {
    ...
    try {
      const response = await api<{ added: number; existing: number; failed: number; items: CheckResult[] }>(
        portalPath("center", "vocabulary", "/center/words/batch"),
        auth.session.access_token,
  ...
  return (
    <>
      ...
      <div className="section">
        <div className="row" style={{ justifyContent: "space-between", marginBottom: 10 }}>
          <h3>Your center contributions</h3>
        ...
        {loadingContributions && contributions.length === 0 ? (
          ...
          <Empty>This learning center has not added vocabulary yet.</Empty>
        ) : (
          <TableWrap>
            <Table>
              <TableHeader>
