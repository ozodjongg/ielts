...
export default function DictionaryPage() {
  ...
  async function search() {
    ...
    setBusy(true);
    try {
      const result = await api<{ items: Lexeme[] }>(portalPath("admin", "vocabulary", `/search?q=${encodeURIComponent(query.trim())}&level=${encodeURIComponent(level)}&limit=50`), auth.session.access_token);
  ...
  async function openSynonyms(index: number) {
    ...
    try {
      const result = await api<{ items: Synonym[] }>(portalPath("admin", "vocabulary", `/words/${index}/synonyms`), auth.session.access_token);
  ...
  return (
    <>
      <PageHeader title="Dictionary explorer" subtitle="Indexed EN→UZ lexemes with CEFR, provenance and synonym graph." />
