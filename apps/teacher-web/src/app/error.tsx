...
export default function GlobalRouteError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("teacher-web route error", error);
  }, [error]);

  return (
    <div className="login-wrap">
      <Card className="login-card">
        <div className="brand"><span className="brandmark">IELTS</span><span>IELTS Teacher</span></div>
        <h1 className="title">Sahifa yuklanmadi</h1>
        <p className="subtitle">Teacher portalda kutilmagan xato yuz berdi.</p>
